...
from topic_modeling.ETM.etm import ETM
from topic_modeling.ETM.utils import nearest_neighbors, get_topic_coherence, get_topic_diversity
...
def evaluate(m, source, tc=False, td=False):
    ...
    with torch.no_grad():
        if source == 'val':
            ...
            tokens = valid_tokens
            counts = valid_counts
            print(f' validation tokens length {len(tokens)}')
        else:
...
if args.mode == 'train':
    ...
    with torch.no_grad():
        ...
        if args.train_embeddings:
            ...
            queries = ['refugee', 'immigrant', 'syria', 'iran']
