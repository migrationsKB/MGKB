...
def create_data_loader(df, tokenizer, max_len, batch_size):
    ...
    :return:
    """
    ### pad the way bert does.
    df = df.dropna(subset=['text_processed'])
    sentences = df.text_processed.values
    sentences_ = ["[CLS] " + sentence + " [SEP]" for sentence in sentences]
    df["sents"]= sentences_
...
#     )
    ...
    lr = 2e-5
    max_grad_norm = 1.0
    num_total_steps = LEN_train_data*EPOCHS
...
def train(dataset, output_dir):
    data_dir ='/home/yiyi/MigrTwi/src/sentiment_analysis/preprocessed_data/pipeline_tp_2/'+dataset
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_accuracy:
            torch.save(model.state_dict(), os.path.join('output', output_dir, 'BERT_{}.bin'.format(str(epoch))))
